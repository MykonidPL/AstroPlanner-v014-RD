...
(function(global){
  ...
  function scoreTarget({raDeg,decDeg,classKey='mixed',metadata=null,effectiveProjectSignal=undefined,materialProfile=null,context,astro,sky=null,minAltitudeDeg=30,sunLimitDeg=-18}){
    ...
    const neutralSignal=Math.abs(signalTimeFactor-1)<1e-12,signalNote=signal.dataStatus==='missing'?`${signal.summary} (${neutralSignal?'wkład neutralny':'dane niepełne; wpływ uwzględniony'})`:signal.dataStatus==='descriptive'?`${signal.summary} (dane opisowe; ${neutralSignal?'wkład neutralny':'wpływ uwzględniony'})`:signal.summary;
